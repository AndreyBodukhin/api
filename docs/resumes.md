...
company_url или null | string | Cайт компании.
...
`code` для `condition_error` принимает значения из списка правил для полей (например, `min_value`, см. [Правила](#condition-rules)).
Мы не можем гарантировать список значений `code` для `business_logic_error`, они могут измениться со временем. 
Вместо этого мы советуем использовать `description` с понятным человеку объяснением ошибки.

Возможные коды `business_logic_error`:

Код | Поле | Пояснение
--- | --- | ---
REQUIRED | skills, skill_set, experience | поле стало обязательным в процессе анализа запроса на изменение
NOT_FOUND | * | не найдено значение по переданному id
NULL_UNIVERSITY_ID_NOT_NULL_FACULTY_ID | education.primary | нельзя установить факультет без университета
AREA_NOT_FOUND | area | не найдено значение area по переданному id
AREA_NOT_A_LEAF | area | значение area не должно содержать потомков (пустой массив areas в объекте из [Дерево всех регионов](areas.md#areas))
END_DATE_BEFORE_START_DATE | experience.end | значение `end` меньше `start`
NOT_COUNTRY | citizenship, work_ticket | значение area должно быть страной (см. [Справочник стран](areas.md#countries))
BLACKLISTED | contact.[] | email или телефон находятся в черном списке
CONTAIN_BLACKLISTED | contact | телефон, указанный в комментарии, находится в черном списке
CANT_BE_SPECIALITY | experience.industry (устаревшее) | переданное значение должно быть отраслью (см. [Отрасли компаний](industries.md))
MUST_BE_SPECIALITY | experience.industries | переданные значения должны быть специальностью (см. [Отрасли компаний](industries.md))
MORE_THAN_ONE_NATIVE_LANGUAGE | language | указано более одного родного языка
MUST_CONTAIN_UNIQUE | language.id, site.type | переданные значения должны быть уникальны
FROM_DIFFERENT_PROFAREAS | specialization | переданные значения из разных отраслей
DUPLICATE | title | значение уже было использовано
BAD_IMAGE_TYPE | portfolio, photo | переданное значение неправильного типа (для portfolio необходимы значения из `GET /artifacts/portfolio`, для photo - `GET /artifacts/photo`) 
BAD_CONTACT_TYPE | contact | не указан предпочитаемый контакт
NEED_COUNTRY_CITY_NUMBER_OR_FORMATTED | contact | телефон в контактах указан в неверном формате (см. [Условия заполнения контактов в резюме](#conditions-contacts))
IS_EMPTY | contact.[].(country, city, number) | не указаны необходимые значения для телефона в контактах
INVALID | contact | невозможно распарсить телефон в контактах
INVALID_URL | experience.company_url, site.url | невозможно распарсить URL
INVALID_SCHEME | experience.company_url, site.url | указанная в URL схема не допустима (разрешены - `http`, `https`)  
INVALID_HOST | experience.company_url, site.url | указанный в URL хост не допустим
UNKNOWN | contact.[].country | указанный код страны в телефоне не определен
NOT_BELONG_THIS_USER | portfolio, photo | используемые сущности не принадлежат текущему пользователю
WRONG_RESIZING_STATE | portfolio, photo | используемые сущности еще не обработаны
CONTAINS_INVALID_CHARACTERS | experience.company_url, site.url | некоторые указанные символы в URL не допустимы
...
Также существуют поля, которые оставлены для обратной совместимости, но 
их не рекомендуется использовать. Для таких полей будет приходить флаг `"deprecated": true`.
Для всех остальных полей - `"deprecated": false`.
